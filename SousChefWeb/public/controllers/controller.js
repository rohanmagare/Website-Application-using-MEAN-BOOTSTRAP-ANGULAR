...
souschefControllers.controller('userPageController',['$scope', function($scope){
    ...
    $(document).ready(function() {
        ...
        $('#dpinput').change(function(){
            ...
            {
              $('#dp').attr('src', '../img/userpagebackground.jpg');
            }
        });
    });
...
$("#signOut").on("click",function(){
  ...
  $http(req).success(function(data){
      if(data.state != 'failure'){
        ...
        setTimeout(modalAnimate($formRegister, $formLogin), 1500);  
...
souschefControllers.controller('checkOutController', ['$scope', '$http', function ($scope,$http,$routeParams){
    $(document).ready(function(){
        $("#mainNav").css('background-color', '#000');
...
souschefControllers.controller('ReviewPurchaseController', ['$scope', '$http', function ($scope,$http){
    $(document).ready(function(){
        $("#mainNav").css('background-color', '#000');
        $("#reviewnav").css('color', '#FFB03B');
    });
    if(JSON.parse(localStorage.getItem("order")) != 'undefined' && JSON.parse(localStorage.getItem("order")) != null){
        //$scope.cart = JSON.parse(localStorage.getItem("order"));
    }
    
}]);

souschefControllers.controller('missionQuotesController', ['$scope', '$http', function ($scope,$http){
    
	$http.get('http://www.json-generator.com/api/json/get/cgJoytinVK?indent=2').success(function(response){
		$scope.missionQuotes=response;
	});
    
}]);

souschefControllers.controller('packageController', ['$scope','$http', function($scope,$http){
...
$http.get('http://www.json-generator.com/api/json/get/bUiZEZhRXC?indent=2').success(function(response){
